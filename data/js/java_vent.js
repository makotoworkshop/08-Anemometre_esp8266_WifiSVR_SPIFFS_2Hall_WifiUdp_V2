      ...
      function drawChart() {    // Fonction principale : 
        ...
        function updateGauge() {     // fonction de mise raffraichissement des données réccupérée dans le fichier json https://www.w3schools.com/jquery/ajax_getjson.asp
          ...
          $.getJSON('mesures.json', function(data)
            {
              $("#VitesseVent").text(data.VitesseVent); // extraire la valeur du json

                  dataGaugeVent.setValue(0, 1, data.VitesseVent); // premier nb pour le N° de la gauge si plusieurs, puis 0 pour modifier le champ label (sinon:1 pour le champ Value), puis la valeur extraite du json
